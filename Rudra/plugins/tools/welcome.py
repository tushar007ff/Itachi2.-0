...
def welcomepic(pic, user, chatname, id, uname):
    background = Image.open("ItachiXMusic/assets/wel2.png")
    pfp = Image.open(pic).convert("RGBA")
    pfp = circle(pfp)
    pfp = pfp.resize((825, 824))
    draw = ImageDraw.Draw(background)
    font = ImageFont.truetype('ItachiXMusic/assets/font.ttf', size=110)
    welcome_font = ImageFont.truetype('ItachiXMusic/assets/font.ttf', size=60)
...
async def greet_group(_, member: ChatMemberUpdated):
    ...
    try:
        ...
        pic = "ItachiXMusic/assets/upic.png"
...
➖➖➖➖➖➖➖➖➖➖➖➖**
""",
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton(f"⦿ ᴀᴅᴅ ᴍᴇ ⦿", url=f"https://t.me/mei_pro_robot?startgroup=true")]])
        )
    except Exception as e:
        LOGGER.error(e)
    try:
